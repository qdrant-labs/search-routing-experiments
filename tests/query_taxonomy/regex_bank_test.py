import pytest
from query_taxonomy.regex_bank import HTTPStatusCodeBank
...
def test_matches_http_keyword_then_code(status_code_bank: HTTPStatusCodeBank):
    assert status_code_bank.matches("HTTP 200") == ['HTTP 200']

def test_matches_http_keyword_then_code_in_prose(status_code_bank: HTTPStatusCodeBank):
    assert status_code_bank.matches("We should send HTTP 200 code") == ['HTTP 200']
...
def test_matches_http_protocol_prefix(status_code_bank: HTTPStatusCodeBank):
    assert status_code_bank.matches("HTTP/1.1 201 CREATED") == ['HTTP/1.1 201']

def test_matches_error_keyword_then_code(status_code_bank: HTTPStatusCodeBank):
    assert status_code_bank.matches("We should send error 500 code") == ['error 500']
...
def test_matches_code_then_reason_and_code_then_informal_reason(status_code_bank: HTTPStatusCodeBank):
    assert status_code_bank.matches("Expected to get 200 OK, got 500 error code") == ['200 OK', '500 error']
