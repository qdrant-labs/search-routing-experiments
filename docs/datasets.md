...
repos, the program's own terms page) for the 42 lanes in
`hybrid_search_rrf_dataset.lanes.LANES`, which collapse to 18 upstream source
families. Re-verify before any release: upstream cards get retagged.
...
- **Pointer artifact, all 42 lanes** — `selected.parquet`,
